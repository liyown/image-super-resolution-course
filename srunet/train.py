...
from utils.model import build_unet
    ...
    for epoch in range(args.num_epochs):
        ...
        with tqdm(total=(len(train_dataset))) as t:
            ...
            for i, data in enumerate(train_dataloader):
                ...
                t.update(len(inputs))
                if i % 1000 == 0:
                    result_srunet["loss"].append(epoch_losses.avg)
        ...
        result_srunet["psnr"].append(epoch_psnr.avg)

        if epoch_psnr.avg > best_psnr:
            best_epoch = epoch
            best_psnr = epoch_psnr.avg
            best_weights = copy.deepcopy(model.state_dict())
    with open(os.path.join(args.outputs_dir, "result_srunet.json"), "w") as f:
        json.dump(result_srunet, f)
